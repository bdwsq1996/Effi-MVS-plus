...
CUDA_VISIBLE_DEVICES=4,5,6,7 python -u train.py --mode='train' --epochs=16 --numdepth=384 --trainviews=5 --testviews=5 --logdir $LOG_DIR --dataset=dtu_yao --batch_size=16 --trainpath=$MVS_TRAINING \
                --trainlist lists/dtu/train.txt --testlist lists/dtu/test.txt --GRUiters 3,3,3 --ndepths 48,8,8 --CostNum=3 --initloss=initloss --lr=0.001 --dispmaxfirst=last  --maskupmode=eachstage --interval_scale=0.53 --lossrate=1.0 | tee -i $dirAndName
#finetune
...
#                --trainlist lists/bld/training_list.txt --testlist lists/bld/validation_list.txt --ndepths=96 --CostNum=3 --initloss=initloss --lr=0.0004 --dispmaxfirst=last  --maskupmode=eachstage --interval_scale=0.53  --lossrate=1.0 | tee -i $dirAndName
